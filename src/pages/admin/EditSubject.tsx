...
function EditSubject() {
  ...
  useEffect(() => {
    if (subject) {
      form.setFieldsValue({
        ...
        subjectDescription: JSON.parse(subject.subjectDescription),
        ...
        sysllabusDescription: JSON.parse(subject.sysllabusDescription),
        studentTask: JSON.parse(subject.studentTask),
        note: JSON.parse(subject.note),
        topic: JSON.parse(subject.topic),
      });
  ...
  const handleSubmit = async (values: CreateSubject) => {
    try {
      ...
      const response = await updateSubject(subjectId, SubjectData);
      console.log(response);

      toast.success("Update Subject Success!", {
        autoClose: 2000,
      });
  ...
  return (
    <div className="flex flex-col px-10 py-5">
      ...
      <div className="text-lg font-semibold h-8 flex gap-2 mb-3">
        ...
        <span className="text-[#2A384D]">
          {" "}
          Edit Subject : {subject?.subjectCode}
        </span>
      </div>
